...
import { cartResponsePage } from './pages/cart-response.ts'
...
app.get('/', c => c.html(layout(homePage(), getCookie(c, 'user'), cartTotal(getCart(c)))))
app.get('/about', c => c.html(layout(aboutPage(), getCookie(c, 'user'), cartTotal(getCart(c)))))
app.get('/store', c => {
  const variant = c.req.query('variant') as 'small' | 'medium' | 'large' | undefined
  return c.html(layout(storePage(variant), getCookie(c, 'user'), cartTotal(getCart(c))))
})
app.get('/login', c => c.html(layout(loginPage(), getCookie(c, 'user'), cartTotal(getCart(c)))))
app.get('/cart', c => {
...
app.post('/cart', async c => {
  ...
  saveCart(c, cart)
  return c.html(cartResponsePage(cartTotal(cart)))
})

app.post('/cart-update', async c => {
  const { variantId, quantity } = await c.req.parseBody<{ variantId: string; quantity: string }>()
